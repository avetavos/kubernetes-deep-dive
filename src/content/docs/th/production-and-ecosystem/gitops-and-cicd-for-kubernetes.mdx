...
**GitOps** ถือว่า Git repository คือ single source of truth ของ desired state ของ cluster โดยมี **reconciler** ที่อยู่ใน cluster (Argo CD, Flux) คอย pull แล้ว apply state นั้นอยู่ตลอด แทนที่ CI job จะ push การเปลี่ยนแปลงด้วย credential ของ cluster ที่ถือเอง pull-based model นี้ทำให้ CI system ไม่ต้องมี access ระดับ cluster-admin เลย การเปลี่ยนแบบ out-of-band ถูกจับได้ผ่าน **drift detection** และประวัติ deployment ทั้งหมดก็คือ **Git history** ธรรมดา pipeline ที่ใช้งานจริงคือ build แล้ว push image ใน CI ก่อน จากนั้น commit image tag ใหม่เข้า repository ของ manifest ให้ reconciler ไปหยิบต่อ
</Callout>
...
export const quizGitOps = [
  ...
  },
  {
    ...
    options: [
      'เข้ารหัส container image ทุกตัวอัตโนมัติ',
      ...
      'ตัด RBAC ออกไปได้ทั้งหมด',
      'ป้องกันไม่ให้ developer เขียน manifest ผิดพลาดได้เลย',
    ],
    answer: 1,
  },
  {
    q: 'ใน GitOps pipeline ที่ใช้งานจริง หลังจาก CI build แล้ว push container image เสร็จ ทำอะไรต่อ',
    options: [
      'รัน kubectl apply เข้า production cluster ตรง ๆ',
      'commit image tag ใหม่เข้า Git repository ที่ GitOps reconciler คอย watch อยู่ แล้วหยุดแค่นั้น',
      'ลบ image ตัวก่อนหน้าออกจาก registry',
      'restart GitOps reconciler ด้วยมือ',
    ],
    answer: 1,
  },
];
