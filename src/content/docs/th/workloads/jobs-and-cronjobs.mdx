...
**Job** รัน Pod จนจบงาน ไม่ใช่รันตลอดไป `completions` กำหนดว่าต้องสำเร็จกี่ครั้ง `parallelism` กำหนดว่ารันพร้อมกันได้กี่ตัว `backoffLimit` กำหนดว่ายอมพังได้กี่ครั้ง และ Pod template ของตัวเองต้องใช้ `restartPolicy: OnFailure` หรือ `Never` **CronJob** create Job ตาม `schedule` โดยมี `concurrencyPolicy` (`Allow`/`Forbid`/`Replace`) คุม run ที่ทับซ้อนกัน และ `successfulJobsHistoryLimit`/`failedJobsHistoryLimit` จำกัดว่าจะเก็บ history ไว้แค่ไหน
</Callout>
