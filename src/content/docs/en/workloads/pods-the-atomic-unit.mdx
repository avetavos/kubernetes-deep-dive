...
A **Pod** is one or more containers sharing a network namespace (one IP, reachable over **localhost**) and optionally volumes; **init containers** run to completion in order before main containers start, and a plain `initContainers` entry with `restartPolicy: Always` becomes a **native sidecar** (GA since v1.33) that keeps running for the Pod's whole life with kubelet-managed startup/shutdown ordering; a Pod's phase (`Pending`/`Running`/`Succeeded`/`Failed`/`Unknown`) and its `restartPolicy` (`Always`/`OnFailure`/`Never`) govern its lifecycle, but a **bare Pod that dies is not recreated** — that job belongs to the controllers covered next.
</Callout>
